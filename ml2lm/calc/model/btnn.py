...
def get_btnn_model(x, y, get_output=get_linear_output, compile_func=compile_default_mse_output, cat_in_dims=None,
                   ...
                   seg_dropout=0.2, fm_dim=320, fm_dropout=0.2, fm_activation=None,
    ...
    if block_num <= 1:
        btnn, extra_inputs = get_tnn_block(
            ...
            fm_dropout=fm_dropout, fm_activation=fm_activation, get_last_layers=get_last_layers,
        ...
        if prev_block_weight_files is None:
            ...
            for i in range(block_num):
                cur_output, extra_inputs = get_tnn_block(
                    i, get_output=get_output, cat_input=cat_input, seg_input=seg_input, num_input=num_input,
                    pre_output=prev_output, cat_in_dims=cat_in_dims, cat_out_dims=cat_out_dims,
                    ...
                    fm_activation=fm_activation, get_last_layers=get_last_layers, hidden_units=hidden_units,
            ...
            btnn = get_btnn_model(
                ...
                fm_activation, get_last_layers, hidden_units, hidden_activation, hidden_dropouts)
            ...
            cur_output, extra_inputs = get_tnn_block(
                block_num - 1, get_output=get_output, cat_input=cat_input, seg_input=seg_input, num_input=num_input,
                pre_output=prev_output, cat_in_dims=cat_in_dims, cat_out_dims=cat_out_dims, seg_out_dims=seg_out_dims,
                num_segs=num_segs, seg_type=seg_type, seg_x_val_range=seg_x_val_range, seg_y_val_range=seg_y_val_range,
                seg_y_dim=seg_y_dim, shrink_factor=shrink_factor, use_fm=use_fm, seg_flag=seg_flag,
